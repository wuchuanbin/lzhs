...
<div class="span9">
    <h1 class="page-title"><?php echo ($title); ?>列表</h1>
    <div class="btn-toolbar">
        <a href="<?php echo U('cate/add'); ?>&flag=<?php echo ($flag); ?>" style='color:#fff;'><button class="btn btn-primary"><i class="icon-plus"></i>新增</button></a>
        <div class="btn-group">
        </div>
    </div>
    <div class="well">
        <table class="table">
            ...
            <tbody>
			<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><tr id="cate_<?php echo ($data["cate_id"]); ?>">
                <td><?php echo ($data["cate_id"]); ?></td>
                <td><?php echo ($data["cate_name"]); ?></td>
                <td>
                    <a href="<?php echo U('cate/edit'); ?>&cid=<?php echo ($data["cate_id"]); ?>&flag=<?php echo ($flag); ?>"><i class="icon-pencil"></i></a>
					<a href="javascript:;" onclick="delcate(<?php echo ($data["cate_id"]); ?>)"><i class="icon-remove"></i></a>
					<input type='hidden' id='delurl' value="<?php echo U('cate/del'); ?>&flag=<?php echo ($flag); ?>" />
                </td>
